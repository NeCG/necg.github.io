  ...
  class RssGenerator < Generator
    ...
    def generate(site)
      ...
      rss = RSS::Maker.make("2.0") do |maker|
        maker.channel.title = site.config['rssname']
        maker.channel.link = site.config['rssurl']
        maker.channel.description = site.config['description'] || "RSS feed for #{site.config['rssname']}"
        ...
        site.posts.reverse[0..post_limit].each do |post|
          maker.items.new_item do |item|
            ...
            item.link = "#{site.config['url']}#{site.config['baseurl'].chomp('/')}#{post.url}"
